...
When using JWT as access tokens, the common usage is to not check with the authorization server
if the JWT is revoked and use short-lived tokens instead.
Because we fear most tokens in real world use of MDS will be long-lived (contrary to the spirit of JWT tokens),
we optionally accept an introspection endpoint that will be called to check token validity in cas it has been revoked
